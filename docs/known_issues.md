...
| Post-save navigation freeze (C3) | All 23 HTML templates loaded Google Fonts via blocking `<link rel="stylesheet">`. Any `location.reload()` (triggered by SSE heat-change listener) blocked page paint until CDN responded — 30-60s at slow-network venues | `fd32a5d` (converted to `media="print" onload` async pattern + preconnect hints; page renders immediately with fallback fonts) |
| comp_run uncached leaderboard | `comp_run()` called `get_leaderboard()` (uncached, full DB query) on every page load, duplicating work already done by `sync_comp_to_broadcast()` | `fd32a5d` (`_get_cached_standings()` used instead) |
| comp_run redundant state reads | `comp_run()` called `load_state()` 3× per request via `_load_category_colors()`, `get_broadcast_mode()`, `set_competition_status()` + `get_all_heats()` full-table scan twice | `fd32a5d` (load state once at top, pass snapshot; replace full scan with COUNT LIMIT 1; skip status write when already "live") |
